          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="p-6">
              <div className="space-y-6">
                ...
                {isLoggedIn && user.id !== product.seller?.id && (
...
function BitcoinPrice({ priceInMxn }) {
...
function BitcoinPayment({ product }) {
      ...
      if (convertData.success) {
        const transactionResponse = await fetch('https://dxcyjoabcqsnpkqalqpd.supabase.co/functions/v1/bitcoin-api/transaction', {
          ...
          body: JSON.stringify({
            ...
            btc_rate: convertData.data.btc_rate
          })
        });
        const transactionData = await transactionResponse.json();
        
        if (transactionData.success) {
          alert('Transacción Bitcoin iniciada. Serás redirigido al procesador de pagos.');
        }
      }
    } catch (error) {
      console.error('Error processing Bitcoin payment:', error);
      alert('Error al procesar el pago con Bitcoin');
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    ...
    >
      {processing ? 'Procesando...' : '🟡 Pagar con Bitcoin'}
    </button>
  );
}
